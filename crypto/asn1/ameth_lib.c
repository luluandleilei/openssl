...
//Returns the public key ASN.1 method idx. The value of idx must be between zero and EVP_PKEY_asn1_get_count() - 1.
...
//looks up the EVP_PKEY_ASN1_METHOD with NID type. If pe isn't NULL, then it will look up an engine implementing a EVP_PKEY_ASN1_METHOD for the NID type and return that instead, and also set *pe to point at the engine that implements it.
...
//Looks up the EVP_PKEY_ASN1_METHOD with PEM type string str. If pe isn't NULL, then it will look up an engine implementing a EVP_PKEY_ASN1_METHOD for the NID type and return that instead, and also set *pe to point at the engine that implements it.
...
//Adds ameth to the user defined stack of methods unless another EVP_PKEY_ASN1_METHOD with the same NID is already there. 
...
int EVP_PKEY_asn1_add0(const EVP_PKEY_ASN1_METHOD *ameth)
{
    ...
    if (sk_EVP_PKEY_ASN1_METHOD_find(app_methods, &tmp) >= 0) {
        EVPerr(EVP_F_EVP_PKEY_ASN1_ADD0, EVP_R_PKEY_APPLICATION_ASN1_METHOD_ALREADY_REGISTERED);
        return 0;
    }
...
//Creates an alias with the NID 'to' for the EVP_PKEY_ASN1_METHOD with NID 'from' unless another EVP_PKEY_ASN1_METHOD with the same NID is already added. 
//This function is not thread safe, it's recommended to only use this when initializing the application.
...
//Returns the public key ID, base public key ID (both NIDs), any flags, the method description and PEM type string associated with the public key ASN.1 method *ameth.
...
//Finds the EVP_PKEY_ASN1_METHOD associated with the key 'pkey'
const EVP_PKEY_ASN1_METHOD *EVP_PKEY_get0_asn1(const EVP_PKEY *pkey)
{
    return pkey->ameth;
}

//Creates and returns a new EVP_PKEY_ASN1_METHOD object, and associates the given id, flags, pem_str and info.
//id is a NID, pem_str is the PEM type string, info is a descriptive string. ASN1_PKEY_SIGPARAM_NULL flag is suppored.
EVP_PKEY_ASN1_METHOD *EVP_PKEY_asn1_new(int id, int flags, const char *pem_str, const char *info)
{
     ...
     */
    if (!((pem_str == NULL && (flags & ASN1_PKEY_ALIAS) != 0) || (pem_str != NULL && (flags & ASN1_PKEY_ALIAS) == 0)))
        goto err;
...
//Copies an EVP_PKEY_ASN1_METHOD object from src to dst. This function is not thread safe, it's recommended to only use this when initializing the application.
